...
namespace Autorepairshop_Case
{
    class Customer : IDatabaseObject
    {
        ...
        {
            int choice = ConsoleHelper.Menu(false, 2, Program.MenuBottom.TopBoundary + 2, Program.MenuBottom.LeftBoundary + 10, "Read/Edit current Customers(Customer ID)", "Read/Edit current Customers(Lastname)", "Create new Customer", "Delete Customer");
            switch (choice)
            {
                case 0:
                    Reader();
        ...
        public static void Deletor()
        {
            ...
            int listIndex = ConsoleHelper.Menu(false, 1, 0, 0, options);
        ...
        {
            Program.Customers = Program.Customers.OrderBy(o => o.Lastname).ToList();
            Reader();
        }
